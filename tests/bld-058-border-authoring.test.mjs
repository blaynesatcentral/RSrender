...
test("BLD-058 region border commits through shared template history authority", async () => {
  const initialJob = job();
  const template = structuredClone(initialJob.template);
  template.regions = template.regions.map((region) =>
    region.id === "region-header"
      ? {
          ...region,
  ...
  const committed = await commitEmbeddedTemplateReplacement(created.session.service, {
    requestId: "urn:rsrender:bld-058:request:region-border:1",
    documentId,
    ownerGeneration: 1,
    expectedWorkingRevision: 0,
    explorationIdentity: boringLogMvpFixture.identity.explorationId,
    expectedEffectiveContentDigest: initialJob.templateDigest,
    replacementEffectiveContentDigest: replacementDigest,
    reason: "Set header Page Region border",
  ...
  assert.ok(captured);
  assert.ok(
    captured.project.aggregate.logSet.embeddedTemplateRepresentations.some(
...
test("BLD-058 rejects malformed borders and exposes human border controls through owned commands", async () => {
  ...
  assert.doesNotMatch(html, /id="apply-region-border"/u);
  ...
  assert.match(renderer, /commitTextBorderControlChange/u);
  assert.match(renderer, /void applySelectedRegionBorder\(\)/u);
});
