...
import { deterministicTextResults } from "../tests/helpers/bld-024-deterministic-text-authority.mjs";
import { PACKAGING_PROFILE } from "./shell-package-bld006.mjs";

const root = path.resolve(import.meta.dirname, "..");
const temporaryRoot = path.join(root, ".tmp", "bld-025-boring-log-studio");
...
const bundleMarker = "rsrender.boring-log-studio.bundle.v1";
...
async function electronZip() {
  const cacheRoot = process.env.LOCALAPPDATA
    ...
    : null;
  if (!cacheRoot) throw new Error("BLD025_ELECTRON_CACHE_UNAVAILABLE");
  const result = await findCachedElectronZip(cacheRoot).catch(() => null);
  if (!result) throw new Error("BLD025_ELECTRON_ZIP_UNAVAILABLE");
  return result;
}

async function resolvedFixtureScene() {
  const stamp = Date.now();
  const sceneModule = await import(
    `${pathToFileURL(path.join(root, "packages", "scene", "dist", "index.js")).href}?bld025=${stamp}`
  );
  const fixtureModule = await import(
    `${pathToFileURL(path.join(root, "packages", "test-support", "dist", "index.js")).href}?bld025=${stamp}`
  );
  const job = {
    contractVersion: 1,
    schemaVersion: "rsrender.boring-log-layout-job.v1",
    kind: "boring-log.layout-job",
    jobId: "job:mvp-boring-log-studio-01@r1",
    inputRevision: 1,
  ...
  const preparation = sceneModule.prepareBoringLogLayout(job);
  if (!preparation.accepted) throw new Error(`BLD025_PAGE_PLAN_REJECTED:${preparation.code}`);
  const result = sceneModule.resolveBoringLogPageScene(
    preparation.value,
    deterministicTextResults(preparation.value.textRequests),
  );
  if (!result.accepted) throw new Error(`BLD025_SCENE_REJECTED:${result.code}`);
  return result.value;
}
...
async function rendererBundle() {
  ...
  if (result.outputFiles.length !== 1) throw new Error("BLD025_RENDERER_BUNDLE_INVALID");
  ...
  for (const forbidden of [
    ...
    if (capabilitySource.includes(forbidden))
      throw new Error(`BLD025_RENDERER_CAPABILITY_INVALID:${forbidden}`);
  }
  return source;
}
...
async function prepareStage() {
  ...
  const scene = await resolvedFixtureScene();
  const rendererUrl = `${pathToFileURL(path.join(root, "packages", "renderer-ui", "dist", "index.js")).href}?bld025=${Date.now()}`;
  const { createBoringLogStudioHtml } = await import(rendererUrl);
  const html = createBoringLogStudioHtml(scene);
  if (typeof html !== "string" || !html.includes('id="resolved-page-scene"')) {
    throw new Error("BLD025_RENDERER_HTML_MISSING");
  }
  const platformUrl = `${pathToFileURL(path.join(root, "packages", "platform-electron-main", "dist", "index.js")).href}?bld025=${Date.now()}`;
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        name: "rsrender-bld025-boring-log-studio",
  ...
  return Object.freeze({
    ...
    stylesheetSha256,
    stylesheetBytes: Buffer.byteLength(stylesheet, "utf8"),
    sceneSha256,
    sceneBytes: Buffer.byteLength(JSON.stringify(scene), "utf8"),
    sceneNodes: scene.pages[0].nodes.length,
    sceneSemantics: scene.pages[0].semanticOrder.length,
    diagnostics: scene.diagnostics.length,
  });
}

export async function packageBoringLogStudio() {
  ...
  if (outputs?.length !== 1 || path.resolve(outputs[0]) !== packagedDirectory) {
    throw new Error("BLD025_UNEXPECTED_PACKAGE_OUTPUT");
  }
  if (sha256(await readFile(packagedExecutable)) !== executableSha256) {
    throw new Error("BLD025_EXECUTABLE_DRIFT");
  }
  const appAsar = path.join(packagedDirectory, "resources", "app.asar");
  return Object.freeze({
    schema: "rsrender.bld025.package-result.v1",
    result: "PASS",
    paths: Object.freeze({
      temporaryRoot,
      stageDirectory,
...
export const BLD025_PATHS = Object.freeze({
  temporaryRoot,
  stageDirectory,
  outputDirectory,
  packagedDirectory,
...
const invokedDirectly = process.argv[1]
  ...
  : false;
if (invokedDirectly) console.log(JSON.stringify(await packageBoringLogStudio()));
