...
import { packageBoringLogStudio } from "./shell-package-bld025.mjs";

const root = path.resolve(import.meta.dirname, "..");
const resultMarker = "RSRENDER_BLD025_RESULT=";
const artifactPath = path.join(root, "artifacts", "bld-025-boring-log-studio-evidence.json");
...
async function runPackaged(packageResult, index) {
  ...
  const profile = path.join(os.tmpdir(), `rsrender-bld025-positive-${index}-profile`);
...
function assertPositive(run, packageResult) {
  ...
  if (
    ...
    result.schema !== "rsrender.bld025.boring-log-studio-probe.v1" ||
    result.result !== "PASS" ||
    ...
    throw new Error(`POSITIVE_STUDIO_INVALID:${run.index}:${JSON.stringify(run)}`);
  }
}
...
async function sourceDigests() {
  const files = [
    ...
    "tooling/shell-package-bld025.mjs",
    "tooling/shell-run-bld025.mjs",
    "tests/bld-025-boring-log-studio.test.mjs",
  ];
  return Object.freeze(
    Object.fromEntries(
      await Promise.all(
...
export async function runBoringLogStudioQualification({ record = false } = {}) {
  const packageResult = await packageBoringLogStudio();
  const runs = [];
  ...
  const evidence = {
    schema: "rsrender.bld025.boring-log-studio-evidence.v1",
    ticket: "BLD-025 / GitHub #69",
    result: "PASS",
    package: packageResult,
    runs,
    claims: {
      packagedExecutable: true,
      rendererNeutralScene: true,
      semanticSvg: true,
      structuredDataToScene: true,
      rasterShortcut: false,
      contentsCanvasPropertiesSelection: true,
      editingAndHistory: false,
      pdfExport: false,
    },
...
const invokedDirectly = process.argv[1]
  ...
  : false;
if (invokedDirectly) {
  const result = await runBoringLogStudioQualification({
