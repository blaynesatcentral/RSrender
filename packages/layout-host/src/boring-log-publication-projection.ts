...
function renderNode(
  ...
  return `<text${common}${attribute("font-family", "RSrender Qualified Arial")}${attribute("font-size", points(measurement.effectiveFontSizeMpt))}${attribute("font-weight", style.fontWeight)}${style.textDecoration === "underline" ? attribute("text-decoration", "underline") : ""}${attribute("fill", style.color)}${attribute("data-font-family-id", style.fontFamilyId)}${attribute("data-text-decoration", style.textDecoration ?? "none")}${attribute("data-authored-font-size-mpt", style.fontSizeMpt)}${attribute("data-effective-font-size-mpt", measurement.effectiveFontSizeMpt)}${attribute("data-font-face-digest", measurement.fontFaceDigest)}${attribute("data-font-metrics-digest", measurement.fontMetricsDigest)}${attribute("data-measurement-id", node.measurementId)}${attribute("data-overflow", measurement.overflow)}${presentationAttributes}>${lines}</text>`;
}
